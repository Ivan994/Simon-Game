...
#define TIMER_SEC 0
#define TIMER_NANO_SEC 1000 * 1000 * 1000 /* 1000ms */
...
char sw0, sw1, sw2, sw3, btn0, btn1;
char sw0T, sw1T, sw2T, sw3T, btn0T, btn1T;

/* Major number. */
int gpio_driver_major;

/* Buffer to store data. */
#define BUF_LEN 1024				//imamo dva bafera. jedan iz app jedan odavde
char *gpio_driver_buffer; //kada upisem u drajver -ovaj je kad upisujemo u njega
unsigned char gpioRead[3]; //kada citam iz drajvera
/* Blink timer vars. */
...
// Check switches to determine if they are in order switched
void CheckSwitches(int sw)
{
	//isli smo dole iz state 1 u state 2, i resetovali smo step na 1
    if(sw != gpio_driver_buffer[step]) //u ovom baferu su nam ti random brojevi
    {
        state = 0;
        lvl = 1;
        step = 1;
        printk(KERN_INFO "Game over\n");
        sw0 = GetGpioPinValue(GPIO_12);	
        sw1 = GetGpioPinValue(GPIO_16);	
        sw2 = GetGpioPinValue(GPIO_20);
        sw3 = GetGpioPinValue(GPIO_21);	
    }else if(sw == gpio_driver_buffer[step])
    {
        printk(KERN_INFO "Step %d DONE!\n", step);
        step++;
    }


}
//ovu funkciju poziva svake sekunde !
...
{
#ifdef TEST
	counter++; //svake sekunde uvecava za 1
	if(state == 1){
		//printk(KERN_INFO "State 1\n");
		if(counter % 2 == 1){		//svake druge sekunde
			if(step <= lvl){
				switch (gpio_driver_buffer[step]) // 1 = 1 krece od 1, ostalih 1023 su random brojevi
				{								//nulti element je state
				case 0:
					SetGpioPin(GPIO_06);
					break;
				case 1:
				...
				step++;
				printk(KERN_INFO "Lvl %d\n", lvl);
			}else{
				step = 1;
				state = 2;
			}
		}else{							//a svaka sekundda koja nije druga, on ocisti pinove ovde
			ClearGpioPin(GPIO_06); 
			ClearGpioPin(GPIO_13);
			ClearGpioPin(GPIO_19);
			ClearGpioPin(GPIO_26);
		}
	}else if(state == 2){ //gotovo je sa pisanjem, sad treba da citamo
        //ove temporary se ucitavaju svake sekunde, i ako su jednaki, ako nismo dirali onda ce biti dobri
        sw0T = GetGpioPinValue(GPIO_12);	
        sw1T = GetGpioPinValue(GPIO_16);	
        sw2T = GetGpioPinValue(GPIO_20);
        sw3T = GetGpioPinValue(GPIO_21);	
        btn0T = GetGpioPinValue(GPIO_03);	
        btn1T = GetGpioPinValue(GPIO_22);

        if(sw0 != sw0T)
        {
            sw0 = GetGpioPinValue(GPIO_12);
            //printk(KERN_INFO "sw0 new value %d\n", sw0);
            CheckSwitches(0);
        }else if (sw1 != sw1T)
        {
            sw1 = GetGpioPinValue(GPIO_16);
            //printk(KERN_INFO "sw1 new value %d\n", sw1);
            CheckSwitches(1);
        }else if(sw2 != sw2T)
        {
            sw2 = GetGpioPinValue(GPIO_20);
            //printk(KERN_INFO "sw2 new value %d\n", sw2);
            CheckSwitches(2);    
        }else if(sw3 != sw3T)
        {
            sw3 = GetGpioPinValue(GPIO_21);
            //printk(KERN_INFO "sw3 new value %d\n", sw3);
            CheckSwitches(3);
        }else if(btn0 != btn0T || btn1 != btn1T)
        {
            btn0 = GetGpioPinValue(GPIO_03);
            btn1 = GetGpioPinValue(GPIO_22);
            //printk(KERN_INFO "btn0 new value %d, btn1 new value: %d\n", btn0,btn1);
        }

        if(step > lvl) //poredi step sa levelom, ako je sve ok idemo na sledeci level
        {
            printk(KERN_INFO "Done lvl %d\n", lvl);
            lvl++;
            step = 1;
            state = 1;
        }
        ...
        printk(KERN_INFO "State 0, start a new game from app!\n");
    }
#endif
...
int gpio_driver_init(void)
{
    ...
    printk(KERN_INFO "SW0 value: %d\n", sw0);
    printk(KERN_INFO "SW1 value: %d\n", sw1);
    printk(KERN_INFO "SW2 value: %d\n", sw2);
    printk(KERN_INFO "SW3 value: %d\n", sw3);
    printk(KERN_INFO "BTN0 value: %d, BTN1 value: %d\n", btn0, btn1);
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
    ...
    memset(gpioRead, 0, 3);
	//ovde cemo u aplikaciji da ispisujemo trenutno stanje, level i korak, stavicmo u aplikaciji svake sekunde..
    gpioRead[0] = state;
    gpioRead[1] = lvl;
    gpioRead[2] = step;
    printk(KERN_INFO "State value: %d\n", gpioRead[0]);
    printk(KERN_INFO "Lvl value: %d\n", gpioRead[1]);
    printk(KERN_INFO "Step value: %d\n", gpioRead[2]);

    if (*f_pos == 0)
    {
        /* Get size of valid data. */
        data_size = strlen(gpioRead);

        /* Send data to user space. */
        if (copy_to_user(buf, gpioRead, data_size) != 0)
        {
            return -EFAULT;
        }
        else
...
{
	// i ovo se poziva iz aplikacije, ova je da upises u bafer.
	// posaljemo iz app 1024  velicinu
	// na prvom mestu je state, pa 1023 su random sekvence
    /* Reset memory. */
    memset(gpio_driver_buffer, 0, BUF_LEN);
	//ovo svaki put kad pozove write iz app on restartuje to, pa posalje novu sekvencu
    ...
    else
    {
        ...
        if(gpio_driver_buffer[0] == 1){
			//upisemo u aplikaciji da nam je to 1, prva pozicija
            state = 1;
            //printk(KERN_INFO "State: %d\n", state);
   
        }else
        {
            printk(KERN_INFO "buffer[0] value must be 1.\n");
        }
